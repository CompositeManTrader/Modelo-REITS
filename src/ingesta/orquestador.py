...
from src.ingesta.instancia import descargar_instancias, rellenar_filing_rezagado
...
def ingestar_estados(
    ...
    *,
    ...
    del_api = mod_estados.hechos_crudos(datos, ticker, desde=desde)
    ...
    # Y el filing que `companyfacts` todavía no ingirió. Va aquí y no solo en la
    # exportación porque si no las dos rutas dejan bases distintas: ingestar se
    # quedaba con el trimestre viejo y solo una exportación posterior lo
    # corregía, en silencio. Se le pasa `del_api` y no el crudo ya concatenado
    # —ver por qué en `rellenar_filing_rezagado`.
    try:
        rezagado = rellenar_filing_rezagado(
            cliente, ticker, cik, del_api, mod_estados.etiquetas_del_catalogo(ticker)
        )
    except ErrorEdgar as exc:
        rezagado = pd.DataFrame()
        resumen.errores.append(f"filing rezagado: {exc}")

    # Los dos caminos leen el MISMO documento cuando la API va atrasada en una
    # emisora con etiquetas propias; se deduplican entre ellos, no contra la API.
    de_instancia = [p for p in (extra, rezagado) if not p.empty]
    if de_instancia:
        instancia = pd.concat(de_instancia, ignore_index=True).drop_duplicates(
            ["tag", "periodo_tipo", "fecha_dato", "fecha_publicacion"]
        )
        crudos = pd.concat([del_api, instancia], ignore_index=True)
    else:
        crudos = del_api
